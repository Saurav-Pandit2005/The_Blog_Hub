...
function AdminArticles({ selectedCategory = "All" }) {
    const navigate = useNavigate();
    const [blogs, setBlogs] = useState([]);
    const [loading, setLoading] = useState(true);
    
    // Get user info from context for like/dashboard logic
    ...
    useEffect(() => {
        const fetchRecentPublications = async () => {
            try {
                ...
                console.error("Error fetching admin home blogs:", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return <div className="loading-state">Syncing Platform Content...</div>;
    }
